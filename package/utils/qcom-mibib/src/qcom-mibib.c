...
static const char *profile_name(enum mr80x_profile profile)
{
	...
	case MR80X_PROFILE_STOCK:
		return "mr80x-v5-stock";
	case MR80X_PROFILE_UNIFIED:
		return "mr80x-v5-unified";
	default:
		return "unknown";
	}
}
...
{
	if (!strcmp(name, "mr80x-v5-stock"))
		return MR80X_PROFILE_STOCK;
	if (!strcmp(name, "mr80x-v5-unified"))
		return MR80X_PROFILE_UNIFIED;
			   ...
			   int *target_block_out)
{
	...
	if (image->size != MIBIB_MIN_SIZE || image->blocks != 4) {
		fprintf(stderr,
			"qcom-mibib: MR80X v5 profile requires an exact 512 KiB MIBIB\n");
		return -1;
	}
...
static void usage(const char *program)
{
	fprintf(stderr,
		...
		"\n"
		"Profiles: mr80x-v5-stock, mr80x-v5-unified\n",
